...
//====================================================================================
public class ConferenceDBTester {
	...
	// -------------------------------------------------------------------------------
	// This is a generic SELECT statement when the user knows how to write SQL
	// code
	// -------------------------------------------------------------------------------
	...
	public static void handleQuery2() {
		System.out.println("Enter status of the student like Freshman, " + "Sophomore, " + "Junior or " + "Senior ");
		String status = sc.nextLine();
		String query = "SELECT BannerId, StudentName FROM STUDENT_15 WHERE Status = '" + status + "';";
		System.out.println(query);
		DatabaseAccessor dbAcc = new DatabaseAccessor();
		dbAcc.setSQLStatement(query);
		Vector<Properties> returnedValues = dbAcc.executeSQLSelectStatement();
	...
	public static void handleQuery4() {
		...
		String query = "SELECT P.ProfessorName, C.CourseName, T.TA_Semester, T.TA_Section "
				...
				+ "WHERE P.ProfessorId = T.ProfessorId AND C.CourseCode = T.CourseCode " + "AND C.DepartmentId = '"
				+ deptId + "';";
		System.out.println(query);
		DatabaseAccessor dbAcc = new DatabaseAccessor();
		dbAcc.setSQLStatement(query);
	...
	public static void handleQuery5() {
		...
		String query = "SELECT S.StudentName, C.CourseName, T.TR_Semester, T.TR_Section, T.TR_Grade "
				...
				+ "WHERE S.BannerId = T.StudentId AND C.CourseCode = T.CourseCode " + "AND T.StudentId = '" + stuId
				+ "';";
		System.out.println(query);
		DatabaseAccessor dbAcc = new DatabaseAccessor();
		dbAcc.setSQLStatement(query);
	...
	public static void handleQuery6() {
		...
		String query = "INSERT INTO DEPARTMENT_15 VALUES ('" + deptId + "', " + deptName + "');";
		System.out.println(query);
	...
	public static void handleQuery7() {
		...
		String query = "UPDATE STUDENT_15 WHERE BannerId = '" + stuId + "' SET Status = '" + newStatus + "');";
		System.out.println(query);
		updateTable(query);
	}
	...
	private static void handleInteractiveQueries() {
		...
		String office = "";
		String query = "";
		String[] englishQuestions = { "Show all the information from Professor table",
				...
				"Insert a new department", "Update a Student Status", "Delete a department" };
		System.out.println("\n\n---------------------------------------------------");
