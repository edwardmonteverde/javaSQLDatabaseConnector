...
public class DatabaseAccessor extends DatabaseManipulator {
	...
	/**
	 * This handles only equality in the WHERE clause and assumes that no type
	 * indication is necessary due to the fact that all values are of Text type
	 * 
	 *
	 * returns a Vector with each element being a Properties object containing
	 * the columnName=columnValue mappings
	 */
	// ------------------------------------------------------------
	public Vector<Properties> getMultiValuedSelectResult(String fieldList, String tableName, Properties whereValues) {
	...
	public Vector<Properties> executeSQLSelectStatement() {
		...
		try {
			...
			if (rs != null) {
				...
				while (rs.next() == true) {
					...
					for (int cnt = 1; cnt <= numRSColumns; cnt++) {
						...
						// The value of the column might be NULL. In that case,
						// we DON'T
			...
			// "executeSQLSelectStatement", "SQL Exception: " +
