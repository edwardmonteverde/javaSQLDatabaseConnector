...
abstract public class DatabaseManipulator {
	 ...
	 * columns, we need to insert the '\\' as escape string in the data values.
	...
	public String insertEscapes(String inString) {
		...
		while (allDone == false) // in other words, there is still an escape
									// char to handle
		{
	...
	/**
	 * The method below returns an object, which either contains a result set if
	 * the associated SQL statement is a SELECT statement, or the return code,
	 * indicating the number of rows inserted, updated or deleted if the
	 * statement is an INSERT, UPDATE or DELETE statement. The return code is
	 * encapsulated in an Integer object.
	 */
	// ------------------------------------------------------------
